...
def test_inference_endpoint_happy_path() -> None:
            ...
            "contexts": [
                {
                    ...
                    "chunk_index": 0,
                    "score": 0.91,
                    "text": "Откройте профиль и выберите пункт сброса пароля",
                }
            ],
        },
    )
    ...
    assert payload["answer"] == "Сбросьте пароль через личный кабинет"

    app.dependency_overrides.clear()


def test_inference_endpoint_validation_error() -> None:
    app.dependency_overrides[get_inference_service] = lambda: FakeInferenceService()
    app.dependency_overrides[require_request_identity] = lambda: RequestIdentity(
        user_subject="user-1",
        user_login="user",
